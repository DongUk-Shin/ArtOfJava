...
// 웹 크롤러 검색
public class MySearchCrawler extends JFrame {
    // 최대 URL 드롭 다운 값.
        ...
        Thread thread = new Thread(new Runnable() {
            public void run() {
                ...
                if (table.getRowCount() == 0) {
                    JOptionPane.showMessageDialog(MySearchCrawler.this, "검색 문자열을 찾을 수 없습니다. 다른 문자열을 시도하세요.", "검색 문자열을 찾을 수 없음", JOptionPane.WARNING_MESSAGE);
                }
            }
        });
        thread.start();
...
//        }
    ...
    private boolean isCrashURL(URL url) {
        try {
            HttpURLConnection connection = (HttpURLConnection) url.openConnection();
            connection.setRequestMethod("GET");
            int responseCode = connection.getResponseCode();
            if (responseCode >= 200 && responseCode < 399) { // 200번대 코드는 성공을 의미
                System.out.println("굿 responseCode = " + responseCode + " | URL: " + url);
                return true;
            } else {
                System.out.println("배드 responseCode = " + responseCode + " | URL: " + url);
                return false;
            }
        } catch (IOException e) {
            // 예외 발생 시 깨진 URL
    ...
    private String downloadPage(URL pageUrl) {
        try {
            ...
            myPageDownload(pageUrl, pageBuffer);
    ...
    private void myPageDownload(URL url, StringBuffer buffer) {
        
        String fileName = url.toString();
        
        String saveFileName = fileName.replaceAll("[/\\\\?*:;|\"<>]", "_");
        
        try {
            OutputStream output = new FileOutputStream( "download/" + saveFileName + ".html", true);
    ...
    public static void main(String[] args) {
        MySearchCrawler crawler = new MySearchCrawler();
        crawler.show();
    }
}
